...
const Dashboard = () => {
            ...
            <div className="space-y-3">
              {jobs.map((j) => (
                <NavLink 
                  key={j.id} 
                  to={`/companies/companyJobs/${j.id}`} 
                  className="block p-5 border border-gray-100 hover:border-blue-200 rounded-xl hover:bg-blue-50/30 transition flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 group shadow-xs"
                >
                  <div className="space-y-1.5">
                    <span className="font-semibold text-gray-900 group-hover:text-blue-600 transition text-lg block">{j.title}</span>
                    {/* Job Status Badge Display */}
                    <div>{renderStatusBadge(j.status || 'pending')}</div>
                  </div>
                  <span className="text-xs font-medium text-gray-400 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100 flex items-center gap-1.5 shrink-0">
                    <Calendar size={14} /> Posted: {new Date(j.created_at).toLocaleDateString('en-US', {
                      year: 'numeric',
                      month: 'short',
                      day: 'numeric'
 ...
 // (Note: keep export default Dashboard; as normal)
export default Dashboard;